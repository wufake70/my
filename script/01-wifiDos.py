...
def set_band_to_monitor(choice):
    """如果你有一个5Ghz网络接口控制器，你可以使用这个功能来放置2.4Ghz或5Ghz频段的显示器，或者两者兼而有之。"""
    if choice == "0":
        # 频带b和g是2.4Ghz WiFi网络
...
def wifi_networks_menu():
    """ 显示无线接入点的循环。我们使用try-except块，然后按ctrl-c退出循环。"""
    ...
    try:
        while True:
            ...
            for file_name in os.listdir():
                    # 我们应该只有一个csv文件，因为我们每次运行程序时都会备份文件夹中以前的所有csv文件。
                    # 以下列表包含csv条目的字段名称。
                    fieldnames = ['BSSID', 'First_time_seen', 'Last_time_seen', 'channel', 'Speed', 'Privacy', 'Cipher', 'Authentication', 'Power', 'beacons', 'IV', 'LAN_IP', 'ID_length', 'ESSID', 'Key']
                    if ".csv" in file_name:
                        with open(file_name) as csv_h:
                            # 我们使用DictReader方法，并告诉它获取csv_h内容，然后使用上面指定的字段名应用字典。
                            # 这将创建一个字典列表，其中包含字段名中指定的关键字。
                            csv_h.seek(0)
            ...
            for index, item in enumerate(active_wireless_networks):
                # 我们使用带有f字符串的print语句。
                # F字符串是在打印字符串时包含变量的一种更直观的方式，#而不是难看的串联。
...
def set_into_managed_mode(wifi_name):
    """将网络控制器接口设置为管理模式并重新启动网络管理器参数：wifi接口名称
    """
    # 将WiFi控制器置于监控模式。
    # 这是将其置于托管模式的一种方法。您也可以使用iwconfig或airmon ng。
    subprocess.run(["ip", "link", "set", wifi_name, "down"])
    # Put the WiFi nic in monitor mode.
...
def deauth_attack(network_mac, target_mac, interface):
    # 我们正在使用aireplay发送一个deauth数据包。
    # 0表示将无限期发送-a用于指定目标路由器的MAC地址-c用于指定我们要发送deauth数据包的mac。
    # 然后我们还需要指定接口
